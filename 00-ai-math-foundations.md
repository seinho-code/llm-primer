...
- 它们加起来不是 `1`
- 以后模型也可能输出负数，例如 `[-3, 5, 1]`
- 分数大小能表示“偏向谁”，但不能直接表示“概率是多少”

所以我们需要一个函数，把“任意实数分数”变成“合法概率分布”。
...
这里的 `e` 是一个固定常数，大约等于 `2.718`。  
你现在不需要深究它为什么是这个数，只需要先把 `exp(x)` 理解成一种“把分数重新映射”的方式。
...
- 不管输入是正是负，`exp(x)` 永远大于 `0`
- 当 `x = 0` 时，`exp(0) = 1`
- `x` 每增加一点，`exp(x)` 会增长得越来越快

为什么会“越来越快”？  
因为指数函数不是线性增长，而是乘法式增长。
...
- 从 `1` 到 `2`，不是只加一点
- 而是在原来基础上又乘了一个大约 `2.718`

也就是说，输入每增加 `1`，输出大约都会乘上同一个倍数 `e`。

这件事对 softmax 很重要，因为模型输出的 logits 本来只是“分数差”。  
经过 `exp` 之后，这种差距会被变成更明显的“筹码差”。

入门时你可以先把 `exp` 的作用总结成两点：

- 它能把任意分数变成正数
- 它会让高分项拿到更多筹码，低分项拿到更少筹码
...
- 原来 `2.0` 只是比 `1.0` 大 `1`
- 指数后 `7.39` 已经明显比 `2.72` 大很多

这很有用，因为模型常常不是只需要“排个序”，而是需要把“更有把握”体现为明显更高的概率。
...
但经过 `exp` 以后，高分项会远远压过低分项。  
这正符合分类和下一个 token 预测的需求，因为模型往往需要表达：

“我不是只稍微偏向这个答案，而是明显更偏向它。”

你也可以把 `exp` 想成一个“放大偏好差距”的函数：

- 原始 logits 只是排名和强弱信号
- `exp(logit)` 把这个信号变成可分配的正筹码
- 后面的归一化，再把这些筹码变成总和为 `1` 的概率

### 第二步：为什么还要除以总和

做完指数后，我们得到的是一组正数，但还不是概率。  
因为它们加起来不一定等于 `1`。
...
如果只看原始分数，你知道第 1 类最高，但不容易直观看出“它到底领先多少”。  
softmax 之后，大致会变成一种很偏向第 1 类的分布：

- 第 1 类很高
- 第 2 类次之
- 第 3 类很低

这说明 softmax 不只是排序，而是在表达：

“模型对第 1 类的信心，比对第 3 类高很多。”

也可以这样理解：

- logits 更像“偏好分数”
- softmax 才把偏好分数翻译成“概率分配”

### 一个特别重要的直觉

softmax 输出的是“相对概率”，不是绝对真理。

比如：

- 如果两个候选的 logits 很接近，softmax 后概率也会接近
- 如果某个候选的 logit 明显更高，softmax 会把更多概率压过去

所以 softmax 的本质是：

“根据每个候选分数在整体中的相对大小，分配总共 100% 的注意力。”

你也可以把它想成“抢份额”：

- 每个候选都先拿到一个由 `exp(logit)` 决定的筹码数
- 然后按筹码占总筹码的比例，分走最终概率

这就是为什么 softmax 特别适合做分类和下一个 token 预测。
