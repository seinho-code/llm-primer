...
这一章比前面更偏论文和系统优化，读的时候不要被术语卡住。  
先抓住主问题：`KV Cache` 太大了，而 TurboQuant 要做的是“压得很狠，但尽量别破坏注意力排序”。

如果你把下面 3 件事读明白，这章就算读到了重点：

- 为什么 KV Cache 是长上下文推理的核心成本
- 为什么压缩 K/V 不能只看数值误差
- TurboQuant 为什么特别强调保住几何关系和在线开销

## 先记住 TurboQuant 在管哪一步

```mermaid
flowchart LR
  A["历史 token 产生 K/V"] --> B["KV Cache"]
  B --> C["压缩/量化"]
  C --> D["后续 attention 读取"]
  D --> E["生成下一个 token"]
```
