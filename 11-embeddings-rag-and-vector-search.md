...
这章最适合带着“企业知识怎么接进模型”这个问题来读。  
如果你一直把大模型理解成“只能靠参数记住知识”，那 embedding 和 RAG 会把这件事彻底改写。

读完这章，你至少应该能说清楚：

- embedding 模型和生成模型的分工
- 一次 RAG 请求从 query 到回答经过了哪些步骤
- 为什么检索质量、切片策略和 rerank 会直接影响最终回答
